...
<div class="main taoxi">
    <div class="breadcrumb"><a href="<?php if($tenantsinfo->shoptype=='婚纱摄影'){ $shoptype='sheying'; }else{ $shoptype='hunli'; }  echo url(Config::get('city.'.$tenantsinfo->city,'北京')).'/'.$shoptype;  ?>">{{$city}}{{$tenantsinfo->shoptype}}</a>&gt;&gt;<a href="{{url('detail')}}/{{$info->tenantsId}}">{{$name}}</a>&gt;&gt; {{$txname}} </div>
    <div class="pic-box">

...
$(document).ready(function() {
	...
	//----------第二次的导航------------//
	mainNav2 = $('.mainnav>ul>li->ul->li');
	console.log(mainNav2);
	mainNav2.find('ul').siblings().addClass('hasUl').append('<span class="hasDrop icon16 icomoon-icon-arrow-down"></span>');
	mainNavLink2 = mainNav2.find('a').not('.sub a');
	mainNavLinkAll2 = mainNav2.find('a');
	mainNavSubLink2 = mainNav2.find('.sub a').not('.sub li .sub a');
	mainNavCurrent2 = mainNav2.find('a.current');
	
	//remove current class if have
	mainNavCurrent2.removeClass('current');
	//set the seleceted menu element
	if ($.cookie("newCurrentMenu2")) {
		mainNavLinkAll2.each(function(index) {
			if($(this).attr('href') == $.cookie("newCurrentMenu2")) {
				//set new current class
				$(this).addClass('current');
				...
				//destroy cookie	
				$.cookie("newCurrentMenu2",null);
			}
		});
	}	
	
	//hover magic add blue color to icons when hover - remove or change the class if not you like.
	mainNavLinkAll2.hover(
	  function () {
	    $(this).find('span.icon16').addClass('blue');
	  }, 
	  function () {
	...
	//click magic
	mainNavLink2.click(function(event) {
		$this = $(this);
		...
		if($this.hasClass('hasUl')) {
			...
			//has no ul so store a cookie for change class.
			$.cookie("newCurrentMenu",$this.attr('href') ,{expires: 1});
			$.cookie("newCurrentMenu2",$this.attr('href') ,{expires: 1});
		}
	});
	mainNavSubLink2.click(function(event) {
		$this = $(this);
